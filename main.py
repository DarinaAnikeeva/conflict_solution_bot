...
f"""
Привет, {user_name}!
Это бот, который может помочь тебе научиться правильно выходить из конфликтов 

Вот, что у меня есть:
#здесь описание каждой кнопки меню и в целом работа бота
#ниже в кнопках посмотреть какие лучше сделать отделы, в каком порядке и тд
""",
        reply_markup=markup
    )
    return States.MENU
...
def feedback_message(update, context) -> States:
    update.message.reply_text(text="Здесь можете написать преложения по улучшениям или другие отзывы")
